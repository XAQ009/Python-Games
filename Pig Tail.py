...
while True:
  players = input( "Enter the number of playaer (2-4): ")
  if players.isdigit():
    players = int(players)
    if 2 <= players <=4:
      break
...
while max(player_score) < max_score:
  ...
  for player_idx in range(players):
    ...
    print("Your totalt score is: ", player_score[player_idx], "\n")
  
    current_score = 0

    while True:
      should_roll = input("would you lie to roll? (y) ")
      if should_roll.lower()!="y":
        break
      value = roll()
      if value == 1:
          print( "\nYou rolled a 1, your turn is done\n")
          current_score+=0
          break
      else:
          current_score += value
